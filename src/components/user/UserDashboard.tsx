...
import { FileText, Clock, CheckCircle, XCircle, Plus, AlertCircle, TrendingUp, DollarSign } from 'lucide-react';
...
export function UserDashboard({ onStartClaim, onViewClaim }: UserDashboardProps) {
  const stats = [
    ...
    { label: 'Total Claimed', value: '$24,500', icon: DollarSign, color: 'text-primary', bg: 'bg-primary/10' },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Quick Action */}
      <div className="bg-gradient-to-r from-primary/20 to-accent/20 border border-primary/30 rounded-2xl p-6 sm:p-8 mb-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-foreground mb-2">Need to file a new claim?</h2>
            <p className="text-muted-foreground">Get started in just a few minutes with our guided process</p>
          </div>
          <button
            onClick={onStartClaim}
            className="bg-primary text-primary-foreground px-6 py-3 rounded-lg flex items-center gap-2 hover:opacity-90 active:scale-[0.98] transition-all whitespace-nowrap"
          >
            <Plus size={20} />
            Start New Claim
          </button>
